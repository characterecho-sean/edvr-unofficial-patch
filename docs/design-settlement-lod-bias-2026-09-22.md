...
Offline measurement, 2026-09-22. No engine code, no flight. Capture: eye
run 165433 (Cranfield, parked on the pad, Pimax OpenXR, 3070x3032 an
eye, game build 332841), frame 2, 18,267 pool eye draws (EB52 10,690).

## Status

- **State:** measured offline; nothing built. The screen-size half of the
  part test is priced exactly and is a weak lever; the half the game's
  own LODDistanceScale slider moves is NOT priced: it needs each part's
  LOD table, which EDVRGATE v2 does not record.
...
- **Open:** the LOD table (param_1[4], 0x80 bytes: t0..t6, count at
  +0x70). Recorded once per *(entry+8) pointer, the tool's bounds become
  exact. That is probe C++ and one parked capture: Sean's call.
- **Ruled out:** see section 6.
- **Next:** Sean's call: close the screen-size half (below the 1.5 ms
  bar); or record the LOD tables (EDVRGATE v3) and re-run `--lod-bias`.
