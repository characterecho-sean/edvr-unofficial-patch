...
- **State:** DECIDED DIRECTION, 2026-09-19 (Sean): engine-level injection from
  ...
  FOLLOW-UPS BUILT, NOT FLOWN, NOT MERGED (branch claude/engine-motion-followups-0938):
  the ~3 ms prep is stage B's coverage pass, now run only when read, and prep's parts
  timed on the price line; a `depth probe layout:` census line; the motion_source
  legend corrected (green = certified rig record, still or moving).
- **Open:** the follow-ups' flight (the re-fly entry's expected price line); on foot
  ...
  UI layer lifts it past DLSS entirely -- remedy for the overseer, in the UI layer's
  and screen motion's files; the stale cockpit (62.8% = the commander's legs under
  unkeyed layers ps_B7D5 and vs_7B0DC42D; keying them is the overseer's call; 35.4%
  undecided until MRT6 is in the eye dump); whether the tracker's extra movers are
  evaluated-but-not-drawn (the census); the boarding flicker (prime candidate: the LOD
  governor, not this arc); stations and ships in space; builder-path movers and
  articulated parts (phase 2); skinned motion's previous palette (phase 2).
- **Ruled out (inherited, do not re-propose):** draw-shape memo identity (~96%
  ...
  pre-pass or a depth bias as the stale cockpit (all three: the re-fly entry). Do not
  re-propose any of the above.
- **Next:** review and merge the follow-ups, then the cockpit leg of the same flight
  (fix.engine_motion=on, dlss, diagnostics 1) read against the re-fly entry: prep's
  parts, the coverage line, `depth probe layout:` on foot. Overseer decisions pending:
  the on-foot 2D screen (a) vs (b), keying ps_B7D5 / vs_7B0DC42D, MRT6 in the eye dump.
