...
- **State:** DECIDED DIRECTION, 2026-09-19 (Sean): engine-level injection from
  ...
  STAGE B REMOVED (part of fix.temporal_aa); PERFORMANCE ROUND merged; ON FOOT flown.
  ...
  NOT FLOWN; vs_4361 waits on its pixel shader's bytecode.
- **Open:** walkers (vs_F516BF0201303B87, unkeyed; w=2 on the panel) wait on phase 2's
  previous bone palette; a temporal pass on the flat source for its own aliasing; the
  stale cockpit (the commander's legs under ps_B7D5 and vs_7B0DC42D, DECIDED not keyed;
  35.4% undecided, low priority); the tracker's evaluated-but-not-drawn movers (the
  09:38 entry); the boarding flicker (the LOD governor, not this arc); the station's
  vs_436193B352A2897E (derives: slot v0.y) and vs_889A5279E68F0672 (derives: v0.x) as
  families; ships in space; builder-path movers and articulated parts (phase 2).
...
- **Next:** a station approach on this build with advanced.glare_shader_dump = 1 (one
  session, restart to arm): stale falls by the keyed share, and ps_16940F576006BE65 is
  dumped -- then vs_4361 is keyed through the harness. Then the controlled diagnostics
  1 vs 0 comparison (the review's protocol).
