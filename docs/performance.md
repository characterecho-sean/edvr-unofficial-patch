...
| Turbo mode | **No, as a performance feature.** It defeats `xrWaitFrame` throttling, which is an OpenXR-runtime behaviour (WMR's, mostly). SteamVR paces through `WaitGetPoses`' running start and does not throttle that way; its own per-app settings already expose what is tunable. Re-implementing pacing is high regression risk for nothing measurable. 2026-09-17: built anyway as `experimental.turbo_mode` (default off), for one question that is not frame rate -- whether the on-foot weapon judder is a pacing race (docs\review-on-foot-weapon-2026-09-11.md, "Turbo mode on foot"); not flown. |
