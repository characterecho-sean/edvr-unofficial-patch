...
| Turbo mode | **No, as a performance feature.** It defeats `xrWaitFrame` throttling, which is an OpenXR-runtime behaviour (WMR's, mostly). SteamVR paces through `WaitGetPoses`' running start and does not throttle that way; its own per-app settings already expose what is tunable. Re-implementing pacing is high regression risk for nothing measurable. 2026-09-17: built anyway, for one question that is not frame rate -- the on-foot weapon judder -- and it answered it: flown on Pimax OpenXR, confirmed, and shipped as what `fix.weapon_stability` now means (deferred pacing while on foot only; in a ship nothing changes). Still not a performance lever: the wait moves from `WaitGetPoses` to the second `Submit`, it does not shrink (docs\review-on-foot-weapon-2026-09-11.md, "Turbo mode promoted"). |
