...
// Edit there, then: python tools/sync_common.py --write   [body-sha256 feea48c31a443ef7]
...
//
// READ ONCE at startup, read only: the two named elements, keyboard entries
// only. A binding on a controller or HOTAS is reported and skipped -- EDVR
// watches the keyboard, and the keyless Status-driven detection covers those
// players instead. An explicit hotkey.* value in edvr.ini always wins over
// what is read here, so nothing changes for anyone who has already set up.
#pragma once
