...
bool menuPanelBuildOverlayContent(MenuContent& c, const char* line, float textDegrees,
    ...
    const int pad = cap * 8 / 10;
    const int em = cap * 10 / 7; // Font::Row, the toast/info text face.
    ...
    // Keep the FPS and optional drop count on the first row. GPU/CPU (or
    // submit-GPU/thread) stay together on the second row, preserving the
    // labels and values generated by perfMonitorOverlayLine.
    std::string first = fields[0];
    std::string second;
    for (size_t i = 1; i < fields.size(); ++i) {
        const bool drop = fields[i].find("dropped") != std::string::npos;
        std::string& dst = drop ? first : second;
        if (!dst.empty()) dst += "   ";
        dst += fields[i];
    }
    ...
    c.capPx = cap;
    c.lineCount = 1;
    if (first.size() >= sizeof(c.lines[0].left) || second.size() >= sizeof(c.lines[1].left))
        return false;
    memcpy(c.lines[0].left, first.c_str(), first.size() + 1);
    c.lines[0].style = kMenuInfo;
    c.lines[0].badge = kBadgeNone;
    // Keep a second row even when startup only says "measuring", so the
    // panel's height and anchor do not jump as samples become available.
    MenuLine& l = c.lines[c.lineCount++];
    if (!second.empty()) memcpy(l.left, second.c_str(), second.size() + 1);
    l.style = kMenuInfo;
    l.badge = kBadgeNone;
    ...
    const char* stable[] = {
        "999 fps   999 dropped",
        "gpu 999.9 ms   cpu 999.9 ms",
        "submit gpu 999.9   thread 999.9",
        "gpu -- ms   cpu -- ms",
        "submit gpu --   thread --",
        "measuring",
    };
