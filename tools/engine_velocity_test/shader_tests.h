...
// MRT6 readback must hold the exact slot (bit 31 masked off) and the exact
...
inline void patchStaticChecks(const Harness& h, const Family& f, const std::vector<BYTE>& vs,
    ...
    h.check(text.find("l(0x7fffffff)") != std::string::npos, "patched PS masks bit 31 off the slot");
...
inline void drawChecks(const Harness& h, const Family& f, const std::vector<BYTE>& vsOriginal,
    ...
    for (UINT i = 0; i < W * H; ++i) {
        ...
        if (slot != 5.0f && slot != 9.0f) { ++wrong; continue; }
        if (zBits == sceneBits) { if (slot == 5.0f) ++covered5; else ++covered9; }
        else ++stale;
    }
