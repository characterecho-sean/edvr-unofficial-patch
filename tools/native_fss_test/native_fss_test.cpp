...
EdvrNativeFssFrame frame(uint64_t seq,uint64_t ref=1){EdvrNativeFssFrame f{};f.size=sizeof(f);f.version=1;f.generation=7;f.referenceGeneration=ref;f.sequence=seq;f.frusta[0][0]=-1.2f;f.frusta[0][1]=.8f;f.frusta[0][2]=-.9f;f.frusta[0][3]=1.1f;f.frusta[1][0]=-.8f;f.frusta[1][1]=1.2f;f.frusta[1][2]=-.9f;f.frusta[1][3]=1.1f;for(unsigned e=0;e<2;++e){f.eyeToHead[e][0]=f.eyeToHead[e][5]=f.eyeToHead[e][10]=1.f;f.eyeToHead[e][3]=e?-.032f:.032f;}return f;}
EdvrNativeFssTable acquire(Device& d){EdvrNativeFssRequest r{sizeof(r),1,d.d.Get(),7};EdvrNativeFssTable t{sizeof(t),1};require(edvrAcquireNativeFss(&r,&t)==S_OK,"acquire");return t;}
...
void run(){
  ...
  mode="heal";arrival=0;chrome=0;auto t=acquire(d);auto f1=frame(1);check(t.beginFrame(t.context,&f1)==S_OK,"begin first");arrival=1;chrome=1;check(treat(t,1,0,left.Get(),full)==S_FALSE,"left first has no donor");fill(d,right.Get(),42);check(treat(t,1,1,right.Get(),full)==S_FALSE,"right consumed");
  arrival=2;chrome=2;auto f2=frame(2);check(t.beginFrame(t.context,&f2)==S_OK,"begin second");fill(d,right.Get(),77);ID3D11Texture2D* healed=nullptr;auto h2=treat(t,2,0,left.Get(),full,&healed);require(h2==S_OK&&healed,"left first heals from previous right");check(healCalls==1&&seenMode==1,"mode one healer");check(std::fabs(seenRect[0]-(.1f+.8f*.5f*(1-131.176f/164.700f)))<.001f&&std::fabs(seenOuter-1.2f)<.001f&&std::fabs(seenInner-.8f)<.001f,"rectangle and tangent order");D3D11_TEXTURE2D_DESC hd{};healed->GetDesc(&hd);check(hd.Width==32&&hd.Height==24,"full ROI output size");check(firstPixel(d,static_cast<ID3D11Texture2D*>(seenRight))==42,"owned donor survives source overwrite");fill(d,right.Get(),99);check(firstPixel(d,static_cast<ID3D11Texture2D*>(seenRight))==42,"donor remains immutable");healed->Release();check(treat(t,2,0,left.Get(),full)==E_INVALIDARG,"duplicate rejected");
  mode="mirror";arrival=3;chrome=3;auto f3=frame(3);check(t.beginFrame(t.context,&f3)==S_OK,"begin mirror");check(treat(t,3,0,left.Get(),full)==S_FALSE,"mirror left raw");check(treat(t,3,1,right.Get(),full)==S_OK&&seenMode==2&&seenLeft!=seenRight,"mirror uses current left");
  unsigned before=healCalls;mode="heal";auto f4=frame(4);check(t.beginFrame(t.context,&f4)==S_OK,"begin gate age");check(treat(t,4,0,left.Get(),full)==S_OK&&healCalls==before+1,"unchanged stamps remain recent");check(treat(t,4,1,right.Get(),full)==S_FALSE,"gate donor refresh");for(uint64_t n=5;n<8;++n){auto fx=frame(n);check(t.beginFrame(t.context,&fx)==S_OK,"begin gate age sequence");check(treat(t,n,0,left.Get(),full)==(n<=6?S_OK:S_FALSE),"arrival gate expires after three frames");check(treat(t,n,1,right.Get(),full)==S_FALSE,"gate donor sequence");}before=healCalls;auto f8=frame(8);check(t.beginFrame(t.context,&f8)==S_OK&&treat(t,8,0,left.Get(),full)==S_FALSE&&healCalls==before,"expired gate stays off");
  mode="heal";arrival=4;chrome=4;auto canted=frame(10);canted.eyeToHead[1][0]=.98f;canted.eyeToHead[1][2]=.2f;canted.eyeToHead[1][8]=-.2f;canted.eyeToHead[1][10]=.98f;check(t.beginFrame(t.context,&canted)==S_OK,"begin canted");check(treat(t,10,0,left.Get(),full)==S_FALSE&&healCalls==before,"canted projection guarded");
  ...
  check(t.invalidate(t.context)==S_OK,"invalidate");auto f12=frame(12);check(t.beginFrame(t.context,&f12)==S_OK,"sequence recovers after invalidate");const float partial[4]={.25f,.25f,.75f,.75f};auto ref=frame(13,2);check(t.beginFrame(t.context,&ref)==S_OK,"reference change clears donors and stamps");arrival=5;chrome=5;check(treat(t,13,1,right.Get(),partial)==S_FALSE,"new reference right donor");arrival=5;chrome=5;auto crop=frame(14,2);check(t.beginFrame(t.context,&crop)==S_OK);ID3D11Texture2D* cropped=nullptr;require(treat(t,14,0,left.Get(),partial,&cropped)==S_OK&&cropped,"ROI heal");D3D11_TEXTURE2D_DESC cd{};cropped->GetDesc(&cd);check(cd.Width==16&&cd.Height==12,"healed output is cropped ROI");cropped->Release();
  ...
  arrival=6;chrome=6;
  check(treat(t,15,1,right.Get(),atlasRight)==S_FALSE,"atlas right donor");
  check(treat(t,15,0,left.Get(),atlasLeft)==S_OK,"equal ROI donors allow different atlas offsets");
  auto changed=frame(16,2);changed.frusta[0][0]=-1.3f;changed.frusta[1][1]=1.3f;
  check(t.beginFrame(t.context,&changed)==S_OK,"begin changed projection");
  before=healCalls;
  check(treat(t,16,0,left.Get(),atlasLeft)==S_FALSE&&healCalls==before,"projection change retires previous donor");
  check(treat(t,16,1,right.Get(),atlasRight)==S_FALSE,"changed projection right donor");
  mode="mirror";changed.sequence=17;
  check(t.beginFrame(t.context,&changed)==S_OK,"begin reversed mirror order");
  check(treat(t,17,1,right.Get(),atlasRight)==S_FALSE&&healCalls==before,"mirror rejects recycled but invalid current left allocation");
  check(treat(t,17,0,left.Get(),atlasLeft)==S_FALSE,"late mirror left remains donor only");
  mode="heal";auto freshReference=frame(18,3);
  check(t.beginFrame(t.context,&freshReference)==S_OK,"begin reference with old nonzero stamps");
  check(treat(t,18,1,right.Get(),full)==S_FALSE&&!published[1],"old stamps do not trigger snapshots after recenter");
  check(treat(t,18,0,left.Get(),full)==S_FALSE&&!published[0]&&healCalls==before,"old stamps cannot heal new reference");
  mode="off";auto disabled=frame(19,3);
  check(t.beginFrame(t.context,&disabled)==S_OK,"begin disabled repair");arrival=7;chrome=7;
  check(treat(t,19,1,right.Get(),full)==S_FALSE&&!published[1],"disabled repair does not publish a donor");
  check(treat(t,19,0,left.Get(),full)==S_FALSE&&!published[0]&&healCalls==before,"disabled repair preserves inputs despite fresh stamps");
