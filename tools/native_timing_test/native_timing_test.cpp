...
static EdvrNativeTimingFrame frame(uint64_t seq) {
    EdvrNativeTimingFrame f{sizeof(f),EDVR_NATIVE_TIMING_VERSION_2,seq};
...
static EdvrNativeTimingTable acquire(ID3D11Device* d,uint64_t gen,Checks& c){
    EdvrNativeTimingRequest r{sizeof(r),EDVR_NATIVE_TIMING_VERSION_2,d,gen};EdvrNativeTimingTable t{sizeof(t),EDVR_NATIVE_TIMING_VERSION_2};
...
int wmain(int argc,wchar_t** argv){
    ...
    EdvrNativeTimingRequest r{sizeof(r)-1,EDVR_NATIVE_TIMING_VERSION_2,d.Get(),1};EdvrNativeTimingTable bad{sizeof(bad),EDVR_NATIVE_TIMING_VERSION_2};c.check(edvrAcquireNativeTiming(&r,&bad)==E_INVALIDARG,"wrong request size");
    r={sizeof(r),EDVR_NATIVE_TIMING_VERSION_2+1,d.Get(),1};c.check(edvrAcquireNativeTiming(&r,&bad)==E_INVALIDARG,"wrong request version");r={sizeof(r),EDVR_NATIVE_TIMING_VERSION_2,d.Get(),1};bad={sizeof(bad)-1,EDVR_NATIVE_TIMING_VERSION_2};c.check(edvrAcquireNativeTiming(&r,&bad)==E_INVALIDARG,"wrong table size");bad={sizeof(bad),EDVR_NATIVE_TIMING_VERSION_2+1};c.check(edvrAcquireNativeTiming(&r,&bad)==E_INVALIDARG,"wrong table version");
    ...
    c.check(t.gpuEye(t.context,s6,0,1,0,tex.Get())==0,"disabled GPU marker reports unavailable");EdvrNativeDeviceGpuSample disabled{sizeof(disabled),EDVR_NATIVE_TIMING_VERSION_2,s6,GetTickCount64(),EdvrNativeGpuDisabled,{0,0},{0,0}};c.check(t.publishDeviceGpu(t.context,&disabled)==S_OK&&edvr::nativeTimingSnapshot().haveDeviceGpu&&edvr::nativeTimingSnapshot().deviceGpu.status==EdvrNativeGpuDisabled,"disabled device GPU status is retained");
