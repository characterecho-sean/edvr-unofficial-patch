...
int guardChild(const char* dir) {
    ...
    // The canted fold. Neither of this project's headsets is canted -- both
    // report 0.00 degrees -- so this seam is the ONLY place the fold can be
    // exercised at all, and everything below is what a canted headset would
    // otherwise have to discover in the air.
    {
        typedef unsigned int(*PFN_Cant)(float, float, float, float, float,
                                        const float*, float*);
        auto cant = reinterpret_cast<PFN_Cant>(
            GetProcAddress(m, "edvr_selftest_canted"));
        if (!cant) {
            printf("  FAIL  guard child: edvr_selftest_canted not exported\n");
            ++bad;
        } else {
            // A symmetric frustum keeps the arithmetic legible: straight
            // ahead lands dead centre when there is nothing to fold in.
            const float fwd[3] = {0.0f, 0.0f, -1.0f};
            float ndc[2] = {9.0f, 9.0f};

            // 1. ZERO CANT IS THE IDENTITY. This is what makes the feature
            //    self-disabling on parallel panels: it installs, folds
            //    nothing, and changes nothing.
            if (cant(0.0f, -1.0f, 1.0f, -1.0f, 1.0f, fwd, ndc) != 1u ||
                fabsf(ndc[0]) > 1e-5f || fabsf(ndc[1]) > 1e-5f) {
                printf("  FAIL  guard child: with zero cant, straight ahead is "
                       "at %g,%g and must be 0,0 -- the fold is not the "
                       "identity on a parallel headset\n", ndc[0], ndc[1]);
                ++bad;
            }

            // 2. THE SIGN, which is the whole hazard. The left eye is turned
            //    OUTWARD (to -x). So a direction straight ahead of the HEAD
            //    is off to that eye's RIGHT -- nasally, +x in NDC. Get this
            //    backwards and the two eyes diverge instead of converging,
            //    which is the images-will-not-fuse failure a commander
            //    described flying with parallel projection off. Exactly
            //    tan(10 deg) on a unit symmetric frustum.
            if (cant(10.0f, -1.0f, 1.0f, -1.0f, 1.0f, fwd, ndc) != 1u ||
                fabsf(ndc[0] - 0.17632698f) > 1e-4f) {
                printf("  FAIL  guard child: with the left eye canted 10 deg "
                       "outward, head-forward lands at NDC x=%g and should be "
                       "+0.176 (tan 10 deg, nasal side). A negative value is "
                       "the sign flip that makes the eyes diverge\n", ndc[0]);
                ++bad;
            }

            // 3. MIRRORED FOR THE OTHER EYE, or the stereo pair is skewed
            //    rather than converged.
            float ndcR[2] = {9.0f, 9.0f};
            if (cant(-10.0f, -1.0f, 1.0f, -1.0f, 1.0f, fwd, ndcR) != 1u ||
                fabsf(ndcR[0] + 0.17632698f) > 1e-4f) {
                printf("  FAIL  guard child: the right eye's cant is not the "
                       "mirror (NDC x=%g, want -0.176)\n", ndcR[0]);
                ++bad;
            }

            // 4. A YAW MOVES NOTHING VERTICALLY. A stray vertical shift here
            //    would be a transposed rotation, which arithmetic alone
            //    cannot tell from the real thing.
            if (fabsf(ndc[1]) > 1e-5f || fabsf(ndcR[1]) > 1e-5f) {
                printf("  FAIL  guard child: a horizontal cant moved the image "
                       "vertically (%g, %g) -- the rotation is transposed\n",
                       ndc[1], ndcR[1]);
                ++bad;
            }

            // 5. THE ROTATION IS A ROTATION, not a shear: a direction that
            //    already points along the canted eye's own axis must land
            //    dead centre. Straight ahead OF THE PANEL, for a panel turned
            //    10 deg toward -x.
            const float a = 10.0f * 0.01745329252f;
            const float alongPanel[3] = {-sinf(a), 0.0f, -cosf(a)};
            if (cant(10.0f, -1.0f, 1.0f, -1.0f, 1.0f, alongPanel, ndc) != 1u ||
                fabsf(ndc[0]) > 1e-4f || fabsf(ndc[1]) > 1e-4f) {
                printf("  FAIL  guard child: the canted eye's own forward axis "
                       "lands at %g,%g instead of the centre -- the fold is "
                       "not a pure rotation\n", ndc[0], ndc[1]);
                ++bad;
            }

            // 6. AN ASYMMETRIC FRUSTUM, because every real one is. The
            //    fixture's own tangents, and the check that the fold composes
            //    with an off-centre projection rather than assuming a
            //    centred one.
            if (cant(0.0f, -1.25f, 0.75f, -1.2f, 0.8f, fwd, ndc) != 1u ||
                fabsf(ndc[0] - 0.25f) > 1e-4f) {
                printf("  FAIL  guard child: on the asymmetric fixture with no "
                       "cant, straight ahead is at NDC x=%g and should be "
                       "+0.25 -- the fold disturbed an off-centre frustum\n",
                       ndc[0]);
                ++bad;
            }
        }
