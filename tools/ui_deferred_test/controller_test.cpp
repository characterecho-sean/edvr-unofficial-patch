...
static std::vector<BYTE> readBuffer(ID3D11Device*d,ID3D11DeviceContext*c,ID3D11Buffer*b){D3D11_BUFFER_DESC bd{};b->GetDesc(&bd);auto staging=bd;staging.Usage=D3D11_USAGE_STAGING;staging.BindFlags=staging.MiscFlags=0;staging.CPUAccessFlags=D3D11_CPU_ACCESS_READ;staging.StructureByteStride=0;ComPtr<ID3D11Buffer>s;hr(d->CreateBuffer(&staging,nullptr,&s));c->CopyResource(s.Get(),b);D3D11_MAPPED_SUBRESOURCE m{};hr(c->Map(s.Get(),0,D3D11_MAP_READ,0,&m));std::vector<BYTE> data(bd.ByteWidth);memcpy(data.data(),m.pData,data.size());c->Unmap(s.Get(),0);return data;}
...
struct Harness {
 ...
 ComPtr<ID3D11VertexShader>vs,postVs,tailVs;ComPtr<ID3D11PixelShader>ui,tone,postTone;
 ComPtr<ID3D11DepthStencilState>noDepth,uiDepth,tailDepth;
 ComPtr<ID3D11BlendState>over,add,destAlpha,unsupported,tailBlend;
 ComPtr<ID3D11RasterizerState>rs;ComPtr<ID3D11SamplerState>sampler;
 ComPtr<ID3D11Buffer>cb,tailIb,tailStructured;
 ComPtr<ID3D11ShaderResourceView>tailStructuredSrv;ComPtr<ID3D11UnorderedAccessView>tailStructuredUav;
 edvr::Surface hdr,ldr,output,expected,expectedHdr;
 ComPtr<ID3D11Texture2D>z,tailZ;ComPtr<ID3D11DepthStencilView>dsv,tailDsv;
 ComPtr<ID3D11ShaderResourceView>zSrv;
 UINT inW=8,inH=8,outW=8,outH=8;
 float bg[4]={.25,.5,.75,1};
 Harness(bool hardware,UINT input=8,UINT out=8,UINT inputHeight=0,UINT outputHeight=0):inW(input),inH(inputHeight?inputHeight:input),outW(out),outH(outputHeight?outputHeight:out){
  ...
  const char*v="cbuffer C:register(b0){float4 colour;float2 jitter;float2 cut;}struct O{float4 p:SV_Position;float2 uv:TEXCOORD0;};O main(uint i:SV_VertexID){O o;float2 p=float2(i==2?3:-1,i==1?3:-1);o.p=float4(p+float2(2*jitter.x,-2*jitter.y),.5,1);o.uv=p*float2(.5,-.5)+.5;return o;}";
   ...
   const char*pv="struct O{float4 p:SV_Position;float2 uv:TEXCOORD0;};O main(uint i:SV_VertexID){float2 q=float2((i&1)?1:-1,(i&2)?1:-1);O o;o.p=float4(q,.5,1);float2 uv=q*float2(.5,-.5)+.5;o.uv=uv*.75+float2(.2,.1);return o;}";
   b=compile(pv,strlen(pv),"main","vs_5_0");hr(d->CreateVertexShader(b->GetBufferPointer(),b->GetBufferSize(),nullptr,&postVs));edvr::uiDeferredRemember(postVs.Get(),b->GetBufferPointer(),b->GetBufferSize(),false);
   const char*tv="cbuffer C:register(b0){float4 colour;float2 jitter;float2 cut;}struct T{float4 value;float4 pad0;float4 pad1;};StructuredBuffer<T> Dynamic:register(t38);struct O{float4 p:SV_Position;float2 uv:TEXCOORD0;};O main(uint i:SV_VertexID){float2 q=float2((i&1)?1:-1,(i&2)?1:-1);O o;o.p=float4(q*.75+float2(2*jitter.x,-2*jitter.y)+float2(Dynamic[0].value.x*.05,0),.5,1);o.uv=q*float2(.5,-.5)+.5;return o;}";
   b=compile(tv,strlen(tv),"main","vs_5_0");hr(d->CreateVertexShader(b->GetBufferPointer(),b->GetBufferSize(),nullptr,&tailVs));edvr::uiDeferredRemember(tailVs.Get(),b->GetBufferPointer(),b->GetBufferSize(),false);
  ...
  b=compile(post,strlen(post),"main","ps_5_0");hr(d->CreatePixelShader(b->GetBufferPointer(),b->GetBufferSize(),nullptr,&postTone));edvr::uiDeferredRemember(postTone.Get(),b->GetBufferPointer(),b->GetBufferSize(),false);shaderHashes[vs.Get()]=0x111;shaderHashes[ui.Get()]=0x222;shaderHashes[tone.Get()]=0x333;shaderHashes[postTone.Get()]=0x444;
   D3D11_BUFFER_DESC bd{};bd.ByteWidth=32;bd.BindFlags=D3D11_BIND_CONSTANT_BUFFER;hr(d->CreateBuffer(&bd,nullptr,&cb));UINT indices[6]={0,1,2,2,1,3};bd={};bd.ByteWidth=sizeof(indices);bd.BindFlags=D3D11_BIND_INDEX_BUFFER;D3D11_SUBRESOURCE_DATA initial{indices,0,0};hr(d->CreateBuffer(&bd,&initial,&tailIb));bd={};bd.ByteWidth=8388624;bd.BindFlags=D3D11_BIND_SHADER_RESOURCE|D3D11_BIND_UNORDERED_ACCESS;bd.MiscFlags=D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;bd.StructureByteStride=48;hr(d->CreateBuffer(&bd,nullptr,&tailStructured));D3D11_SHADER_RESOURCE_VIEW_DESC structuredView{};structuredView.Format=DXGI_FORMAT_UNKNOWN;structuredView.ViewDimension=D3D11_SRV_DIMENSION_BUFFER;structuredView.Buffer.NumElements=174763;hr(d->CreateShaderResourceView(tailStructured.Get(),&structuredView,&tailStructuredSrv));D3D11_UNORDERED_ACCESS_VIEW_DESC structuredUav{};structuredUav.Format=DXGI_FORMAT_UNKNOWN;structuredUav.ViewDimension=D3D11_UAV_DIMENSION_BUFFER;structuredUav.Buffer.NumElements=174763;hr(d->CreateUnorderedAccessView(tailStructured.Get(),&structuredUav,&tailStructuredUav));UINT zero[4]{};c->ClearUnorderedAccessViewUint(tailStructuredUav.Get(),zero);
   ...
   dd={};dd.DepthEnable=FALSE;dd.StencilEnable=TRUE;dd.StencilReadMask=0;dd.StencilWriteMask=4;dd.FrontFace=dd.BackFace={D3D11_STENCIL_OP_KEEP,D3D11_STENCIL_OP_KEEP,D3D11_STENCIL_OP_REPLACE,D3D11_COMPARISON_ALWAYS};hr(d->CreateDepthStencilState(&dd,&tailDepth));
   ...
   r.SrcBlend=r.SrcBlendAlpha=D3D11_BLEND_ONE;r.DestBlend=r.DestBlendAlpha=D3D11_BLEND_INV_SRC_ALPHA;r.RenderTargetWriteMask=7;hr(d->CreateBlendState(&blend,&tailBlend));
 ...
 void uiDraw(float jx,float jy,ID3D11BlendState*blend,bool defer){constants(jx,jy);setup(hdr,inW,inH,ui.Get(),blend,dsv.Get());edvr::uiDeferredTraceDrawEnter(c.Get(),true,'N',3,1,0,vsHash,psHash);edvr::uiDeferredBeforeDraw(c.Get(),'N',3,1,0,0,0,0);bool captured=defer&&edvr::uiDeferredBegin(c.Get(),0,'N',3,1,0,0,0);check(!defer||captured,"recognized UI captured");c->DrawInstanced(3,1,0,0);edvr::uiDeferredTraceOriginalIssued();if(captured)edvr::uiDeferredEnd(c.Get());}
 void toneDraw(bool defer,uint64_t variant=edvr::EyeTonemapSnapshot::kVs){constants(0,0);setup(ldr,inW,inH,tone.Get(),nullptr,nullptr);c->PSSetShaderResources(1,1,hdr.srv.GetAddressOf());vsHash=variant;psHash=edvr::EyeTonemapSnapshot::kPs;edvr::uiDeferredTraceDrawEnter(c.Get(),true,'N',3,1,0,vsHash,psHash);edvr::uiDeferredBeforeDraw(c.Get(),'N',3,1,0,0,0,0);if(defer){edvr::uiDeferredTraceBeforeTone(c.Get());edvr::uiDeferredBeforeTone(c.Get(),'N',3,1,0,0,0);check(!edvr::uiDeferredBegin(c.Get(),-1,'N',3,1,0,0,0),"full wrapper tone has no later world mirror");}c->DrawInstanced(3,1,0,0);edvr::uiDeferredTraceOriginalIssued();if(defer)edvr::uiDeferredEnd(c.Get());}
 void candidateDraw(edvr::Surface&target,ID3D11ShaderResourceView*source,uint64_t vh,uint64_t ph){constants(0,0);setup(target,inW,inH,postTone.Get(),nullptr,nullptr);c->PSSetShaderResources(0,1,&source);vsHash=vh;psHash=ph;edvr::uiDeferredTraceDrawEnter(c.Get(),true,'N',3,1,0,vh,ph);edvr::uiDeferredBeforeDraw(c.Get(),'N',3,1,0,0,0,0);edvr::uiDeferredTraceBeforeTone(c.Get());c->DrawInstanced(3,1,0,0);edvr::uiDeferredTraceOriginalIssued();edvr::uiDeferredEnd(c.Get());}
 void postToneDraw(edvr::Surface&target,ID3D11ShaderResourceView*source,bool allow=true){constants(0,0);setup(target,inW,inH,postTone.Get(),nullptr,nullptr);c->VSSetShader(postVs.Get(),nullptr,0);c->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);c->PSSetShaderResources(0,1,&source);vsHash=edvr::kPostToneVs;psHash=edvr::kPostTonePs;edvr::uiDeferredTraceDrawEnter(c.Get(),true,'N',4,1,0,vsHash,psHash);edvr::uiDeferredBeforeDraw(c.Get(),'N',4,1,0,0,0,allow?0:7);edvr::uiDeferredTraceBeforeTone(c.Get());edvr::uiDeferredBegin(c.Get(),-1,'N',4,1,0,0,0);c->DrawInstanced(4,1,0,0);edvr::uiDeferredTraceOriginalIssued();edvr::uiDeferredEnd(c.Get());}
 bool tailDraw(edvr::Surface&target,ID3D11ShaderResourceView*source,float jx,float jy,uint32_t verdict=0){constants(jx,jy);setup(target,inW,inH,postTone.Get(),tailBlend.Get(),tailDsv.Get());c->OMSetDepthStencilState(tailDepth.Get(),4);c->VSSetShader(tailVs.Get(),nullptr,0);c->VSSetShaderResources(38,1,tailStructuredSrv.GetAddressOf());c->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);c->IASetIndexBuffer(tailIb.Get(),DXGI_FORMAT_R32_UINT,0);c->PSSetShaderResources(0,1,&source);vsHash=edvr::kTailVs;psHash=edvr::kTailPs;edvr::uiDeferredTraceDrawEnter(c.Get(),true,'X',6,1,verdict,vsHash,psHash);edvr::uiDeferredBeforeDraw(c.Get(),'X',6,1,0,0,0,verdict);edvr::uiDeferredTraceBeforeTone(c.Get());const bool handled=edvr::uiDeferredBegin(c.Get(),0,'X',6,1,0,0,0);c->DrawIndexedInstanced(6,1,0,0,0);edvr::uiDeferredTraceOriginalIssued();edvr::uiDeferredEnd(c.Get());return handled;}
...
int main(int argc,char**argv){
 ...
 if(argc>1 && strcmp(argv[1],"--benchmark")==0){
  Harness h(true,2644,4068,2610,4016);auto*c=h.c.Get();auto*d=h.d.Get();auto post=surf(d,2644,2610,DXGI_FORMAT_R8G8B8A8_UNORM),tailSource=surf(d,512,512,DXGI_FORMAT_R8G8B8A8_UNORM);
 ...
 for(UINT scale:{1u,2u})for(float j:{0.f,.25f,-.25f}){
  ...
  if(scale==1 && j==0){
   UINT first[4]={1,2,3,4},second[4]={5,6,7,8};D3D11_BUFFER_DESC bd{};bd.ByteWidth=sizeof(first);bd.BindFlags=D3D11_BIND_SHADER_RESOURCE|D3D11_BIND_UNORDERED_ACCESS;bd.MiscFlags=D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;bd.StructureByteStride=sizeof(UINT);D3D11_SUBRESOURCE_DATA initial{first,0,0};ComPtr<ID3D11Buffer>source;hr(d->CreateBuffer(&bd,&initial,&source));ComPtr<ID3D11Resource>before,after;check(!edvr::snapshots.capture(c,source.Get(),before,true),"ordinary reflected capture still rejects UAV-capable buffers");check(edvr::snapshots.capture(c,source.Get(),before,true,true),"terminal opt-in supports UAV-capable reflected structured buffer snapshot");c->UpdateSubresource(source.Get(),0,nullptr,second,0,0);check(edvr::snapshots.capture(c,source.Get(),after,true,true),"opted-in reflected buffer can be captured again without write-hook evidence");ComPtr<ID3D11Buffer>beforeBuffer,afterBuffer;hr(before.As(&beforeBuffer));hr(after.As(&afterBuffer));D3D11_BUFFER_DESC copyDesc{};beforeBuffer->GetDesc(&copyDesc);check(before.Get()!=after.Get() && copyDesc.ByteWidth==sizeof(first) && copyDesc.StructureByteStride==sizeof(UINT) && copyDesc.MiscFlags==D3D11_RESOURCE_MISC_BUFFER_STRUCTURED && copyDesc.BindFlags==D3D11_BIND_SHADER_RESOURCE,"volatile reflected UAV buffer uses fresh SRV-only structured clones");check(readBuffer(d,c,beforeBuffer.Get())==std::vector<BYTE>((BYTE*)first,(BYTE*)first+sizeof(first)) && readBuffer(d,c,afterBuffer.Get())==std::vector<BYTE>((BYTE*)second,(BYTE*)second+sizeof(second)),"fresh reflected UAV snapshots preserve each captured buffer version");
  }
  {
  ...
  }
  {
   ...
   check(edvr::diagnostic.postToneCandidates==afterTone.postToneCandidates+1,"post-tone sampled blit diagnostic is wired");
    check(edvr::eyes[0].complete && edvr::eyes[0].postTone.ready() && edvr::eyes[0].aliases.front().Get()==post.tex.Get(),"sampled blit advances the alias to its exact destination");
    auto*sampledClean=edvr::uiDeferredPrepare(c,post.tex.Get(),h.zSrv.Get(),h.output.tex.Get(),0,8,8,j,-j);check(sampledClean,"sampled blit destination prepares through retained route");ComPtr<ID3D11Resource>sampledResource;sampledClean->GetResource(&sampledResource);check(sampledResource.Get()==edvr::eyes[0].cleanFinal.tex.Get(),"sampled route publishes clean final rather than pre-blit tone");
  }
  {
   h.start();h.uiDraw(j,-j,h.over.Get(),true);h.toneDraw(true,edvr::kToneVsConstantExposure);check(edvr::eyes[0].complete && edvr::eyes[0].tone.originalVertexShader()==h.vs.Get(),"constant-exposure tone variant captures its live vertex shader");
   h.start();h.uiDraw(j,-j,h.over.Get(),true);const auto observations=edvr::diagnostic.toneObservations;h.toneDraw(true,0x123456789ABCDEF0ull);check(!edvr::eyes[0].complete && !edvr::failed && edvr::diagnostic.toneObservations==observations,"unknown tone vertex variant is ignored without weakening fallback");h.toneDraw(true);check(edvr::eyes[0].complete,"known sampled-exposure tone remains accepted after unknown variant");
  }
  {
   h.start();h.uiDraw(j,-j,h.over.Get(),true);h.toneDraw(true);auto post=surf(d,8,8,DXGI_FORMAT_R8G8B8A8_UNORM);h.postToneDraw(post,h.ldr.srv.Get(),false);
   check(edvr::eyes[0].complete && !edvr::eyes[0].postTone.ready() && edvr::eyes[0].aliases.front().Get()==h.ldr.tex.Get(),"nonzero-verdict sampled draw cannot advance route alias or retain packet");
  }
  {
   h.start();h.uiDraw(j,-j,h.over.Get(),true);h.toneDraw(true);auto post=surf(d,8,8,DXGI_FORMAT_R8G8B8A8_UNORM),tailSource=surf(d,8,8,DXGI_FORMAT_R8G8B8A8_UNORM),tailOriginal=surf(d,8,8,DXGI_FORMAT_R8G8B8A8_UNORM),mappedExpected=surf(d,h.outW,h.outH,DXGI_FORMAT_R8G8B8A8_UNORM),mappedTransmission=surf(d,h.outW,h.outH,DXGI_FORMAT_R32_FLOAT);
   h.postToneDraw(post,h.ldr.srv.Get());const float tailColour[4]={.8f,.1f,.05f,.5f};c->ClearRenderTargetView(tailSource.rtv.Get(),tailColour);c->CopyResource(tailOriginal.tex.Get(),tailSource.tex.Get());check(h.tailDraw(post,tailSource.srv.Get(),j,-j),"exact terminal canvas consumes handled token and suppresses legacy decline");
   check(edvr::eyes[0].tails.size()==1 && edvr::diagnostic.tailCaptures,"terminal canvas packet is retained once");ComPtr<ID3D11Resource>capturedStructured;edvr::eyes[0].tails[0]->vsResource(38)->GetResource(&capturedStructured);ComPtr<ID3D11Buffer>capturedStructuredBuffer;hr(capturedStructured.As(&capturedStructuredBuffer));D3D11_BUFFER_DESC capturedDesc{};capturedStructuredBuffer->GetDesc(&capturedDesc);check(capturedDesc.ByteWidth==8388624 && capturedDesc.StructureByteStride==48 && capturedDesc.MiscFlags==D3D11_RESOURCE_MISC_BUFFER_STRUCTURED && capturedDesc.BindFlags==D3D11_BIND_SHADER_RESOURCE,"terminal packet captures exact-size UAV-capable t38 as an SRV-only structured snapshot");auto originalGame=read(d,c,post.tex.Get());
   edvr::uiDeferredResourceWrite(c,tailSource.tex.Get());const float changed[4]={.05f,.9f,.2f,.9f};c->ClearRenderTargetView(tailSource.rtv.Get(),changed);UINT changedStructured[4]={0x41200000u,0x41200000u,0x41200000u,0x41200000u};c->ClearUnorderedAccessViewUint(h.tailStructuredUav.Get(),changedStructured);h.constants(-j,j,.9f);
   auto*clean=edvr::uiDeferredPrepare(c,post.tex.Get(),h.zSrv.Get(),h.output.tex.Get(),0,8,8,j,-j);check(clean,"sampled route with terminal canvas prepares");float world[4]={h.bg[0]/(1+h.bg[0]),h.bg[1]/(1+h.bg[1]),h.bg[2]/(1+h.bg[2]),1};c->ClearRenderTargetView(h.output.rtv.Get(),world);edvr::uiDeferredApply(c,0);check(read(d,c,post.tex.Get())==originalGame,"native tail replay leaves original game canvas unchanged");
   h.baselineNative(h.over.Get());h.constants(0,0);h.setup(mappedExpected,h.outW,h.outH,h.postTone.Get(),nullptr,nullptr);c->VSSetShader(h.postVs.Get(),nullptr,0);c->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);c->PSSetShaderResources(0,1,h.expected.srv.GetAddressOf());c->DrawInstanced(4,1,0,0);
   h.constants(0,0);h.setup(mappedExpected,h.outW,h.outH,h.postTone.Get(),h.tailBlend.Get(),nullptr);c->VSSetShader(h.tailVs.Get(),nullptr,0);c->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);c->IASetIndexBuffer(h.tailIb.Get(),DXGI_FORMAT_R32_UINT,0);c->PSSetShaderResources(0,1,tailOriginal.srv.GetAddressOf());c->DrawIndexedInstanced(6,1,0,0,0);
   auto routed=read(d,c,h.output.tex.Get()),anchored=read(d,c,mappedExpected.tex.Get());size_t routeDifferences=0;int routeMax=0;for(size_t i=0;i<routed.size();++i){const int delta=abs(int(routed[i])-int(anchored[i]));if(delta>1)++routeDifferences;routeMax=(std::max)(routeMax,delta);}if(routeDifferences){fprintf(stderr,"sampled/tail route scale=%u jitter=%g differences=%zu max=%d\n",scale,j,routeDifferences,routeMax);for(size_t p=0;p<routed.size()/4;++p)if(memcmp(routed.data()+p*4,anchored.data()+p*4,4))fprintf(stderr," p%zu (%u,%u,%u,%u)/(%u,%u,%u,%u)\n",p,routed[p*4],routed[p*4+1],routed[p*4+2],routed[p*4+3],anchored[p*4],anchored[p*4+1],anchored[p*4+2],anchored[p*4+3]);}check(!routeDifferences,"native sampled mapping and captured tail match anchored zero-jitter baseline after source/CB mutation");
   h.setup(mappedTransmission,h.outW,h.outH,h.postTone.Get(),nullptr,nullptr);c->VSSetShader(h.postVs.Get(),nullptr,0);c->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);c->PSSetShaderResources(0,1,edvr::renderer.transmission.srv.GetAddressOf());c->DrawInstanced(4,1,0,0);
   const auto mappedMask=read(d,c,edvr::renderer.mappedTransmission.tex.Get()),rawMask=read(d,c,edvr::renderer.transmission.tex.Get());check(mappedMask!=rawMask && mappedMask==read(d,c,mappedTransmission.tex.Get()),"nonidentity sampled draw maps transmission coverage with the same UV and sampler as colour");
  }
  {
   h.start();h.uiDraw(0,0,h.over.Get(),true);h.toneDraw(true);auto post=surf(d,8,8,DXGI_FORMAT_R8G8B8A8_UNORM),tailSource=surf(d,8,8,DXGI_FORMAT_R8G8B8A8_UNORM);h.postToneDraw(post,h.ldr.srv.Get());
   for(size_t i=0;i<edvr::kTailDrawLimit;++i)check(h.tailDraw(post,tailSource.srv.Get(),0,0),"bounded terminal packet accepted");check(edvr::eyes[0].tails.size()==edvr::kTailDrawLimit,"terminal packet count reaches exact bound");check(!h.tailDraw(post,tailSource.srv.Get(),0,0) && edvr::failed,"terminal packet beyond bound declines to original game colour");
  }
  {
  ...
  for(auto* blend:{static_cast<ID3D11BlendState*>(nullptr),h.add.Get(),h.over.Get(),h.destAlpha.Get()}){
   auto world=[&](bool mirror){h.constants(0,0,.25);h.setup(h.hdr,8,8,h.ui.Get(),blend,h.dsv.Get());edvr::uiDeferredBeforeDraw(c,'N',3,1,0,0,0,0);if(mirror)check(!edvr::uiDeferredBegin(c,-1,'N',3,1,0,0,0),"world is mirrored without UI classification");c->DrawInstanced(3,1,0,0);edvr::uiDeferredEnd(c);};
