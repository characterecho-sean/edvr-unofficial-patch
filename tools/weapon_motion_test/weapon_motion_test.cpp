...
struct Pose {float mouse=0,projection=1,nearBone=0,farBone=0;};
F4 vertex(int i,Pose p){const double x=i==0||i==3?-.6:.6,y=i<2?-.6:.6,t=i<2?0:1;double z=1+t*.25;return {(x+p.mouse+p.nearBone*(1-t)+p.farBone*t)*p.projection,y,.025,z};}
int main(int argc,char** argv){
 ...
 const char* source="cbuffer P:register(b1){float mouse,projection,a,b;} StructuredBuffer<float4> Bones:register(t38);struct O{uint2 extra:DATA0;float4 normal:DATA1;float4 p:SV_Position;};O main(float4 p:POSITION){O o;o.extra=0;o.normal=1;o.p=float4((p.x+mouse+lerp(Bones[0].x,Bones[1].x,p.w))*projection,p.y,.025,p.z);return o;}";
 ...
 auto vb=buffer(vertices,sizeof(vertices),D3D11_BIND_VERTEX_BUFFER),ib=buffer(indices,sizeof(indices),D3D11_BIND_INDEX_BUFFER),cb=buffer(nullptr,16,D3D11_BIND_CONSTANT_BUFFER),bones=buffer(nullptr,32,D3D11_BIND_SHADER_RESOURCE,16);
 ComPtr<ID3D11ShaderResourceView> boneView;hr(dev->CreateShaderResourceView(bones.Get(),nullptr,&boneView));
 auto bind=[&](Pose p){weaponMotionFrameBoundary();weaponMotionSource(depth.Get());ctx->OMSetRenderTargets(1,rtv.GetAddressOf(),dsv.Get());ctx->OMSetDepthStencilState(state.Get(),21);ctx->ClearDepthStencilView(dsv.Get(),D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL,0,4);D3D11_VIEWPORT vp{0,0,float(W),float(H),0,1};ctx->RSSetViewports(1,&vp);ctx->IASetInputLayout(layout.Get());UINT stride=16,off=0;ctx->IASetVertexBuffers(1,1,vb.GetAddressOf(),&stride,&off);ctx->IASetIndexBuffer(ib.Get(),DXGI_FORMAT_R32_UINT,0);ctx->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);ctx->VSSetShader(vs.Get(),nullptr,0);ctx->PSSetShader(ps.Get(),nullptr,0);ctx->VSSetConstantBuffers(1,1,cb.GetAddressOf());ctx->UpdateSubresource(cb.Get(),0,nullptr,&p,0,0);float b[8]={p.nearBone,0,0,0,p.farBone,0,0,0};ctx->UpdateSubresource(bones.Get(),0,nullptr,b,0,0);ctx->VSSetShaderResources(38,1,boneView.GetAddressOf());issue(ctx.Get(),9,1,0,0,0);};
 ...
 // Duplicate geometry cannot be identified by a volatile instance slot.
 motion();check(!weaponMotionView(),"duplicate mesh identity rejects frame");bind(old);motion();check(weaponMotionView()!=nullptr,"next frame recovers from ambiguity");
